(function() {
  ...
  chatWidgetContainer.innerHTML = `
    ...
    <div id="chat-popup" class="hidden absolute bottom-20 right-0 w-96 bg-white rounded-md shadow-md flex flex-col transition-all text-sm">
      <div id="chat-header" class="flex justify-between items-center p-4 bg-gray-800 text-white rounded-t-md">
        <h3 class="m-0 text-lg">CPF Rafiki Online Assistant</h3>
      ...
      <div id="chat-input-container" class="p-4 border-t border-gray-200">
        ...
        <div class="flex text-center text-xs pt-4">
          <span class="flex-1">Copyright @ <a href="https://www.cpf.or.ke" target="_blank" class="text-indigo-600">CPF Financial Services Ltd</a></span>
        </div>
      </div>
    </div>
  `;
  ...
  function onUserRequest(message) {
    ...
    let jsonData = `{"username" : "1234567891", "messageContent": "${message}"}`;
    xmlhttp.send(jsonData);
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 201) {
      
        var responseJsonObj = JSON.parse(this.responseText);
        reply(responseJsonObj.responseMessage);
